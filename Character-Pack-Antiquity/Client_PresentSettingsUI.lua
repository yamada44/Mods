	...
	local vert = UI.CreateVerticalLayoutGroup(rootParent);
		...
		for i = 1, Mod.Settings.BeforeMax  do 
			...
			if (Mod.Settings.Unitdata[i].Maxunits == 0) then goto next end
				...
				UI.CreateLabel(vert).SetText('Attack Power in armies: ' .. Mod.Settings.Unitdata[i].unitpower).SetColor('#FFDDF4')
				UI.CreateLabel(vert).SetText('Defence power in armies: ' .. defend);
				UI.CreateLabel(vert).SetText('Max amount at once: ' .. Mod.Settings.Unitdata[i].Maxunits).SetColor('#FFDDF4')
				UI.CreateLabel(vert).SetText('Min possible age range: ' .. Mod.Settings.Unitdata[i].Minlife)
				UI.CreateLabel(vert).SetText('Max possible age range: ' .. Mod.Settings.Unitdata[i].Maxlife).SetColor('#FFDDF4')
				UI.CreateLabel(vert).SetText('Unit tranfered upon death Amount: ' .. transfer);
				UI.CreateLabel(vert).SetText('Max amount of units allowed to be spawned over entire game: ' .. MaxServer).SetColor('#FFDDF4')
				UI.CreateLabel(vert).SetText('Base Number of armies needed to kill to level up: ' .. level);
				UI.CreateLabel(vert).SetText('Unit locked till turn: ' .. active).SetColor('#FFDDF4')
				UI.CreateLabel(vert).SetText('Shared Max between players: ' .. Shared);
				UI.CreateLabel(vert).SetText('Visible to all players: ' .. Vis).SetColor('#FFDDF4')
				UI.CreateLabel(vert).SetText('Move on Even turns only: ' .. even);

				UI.CreateLabel(vert).SetText('Image used: ' .. image).SetColor('#FFDDF4')
