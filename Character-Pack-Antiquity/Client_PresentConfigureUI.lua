...
UnitTypeMax = InputFieldTable.UnitTypeMax.GetValue()
    ...
    InputFieldTable[i].costInputField = UI.CreateNumberInputField(row1)
		...
		.SetValue(cost);
		CreateButton(row1).SetText("?").SetColor('0000FF').SetOnClick(function() UI.Alert("This unit gains attack power equal to the unit below"); end);
...
InputFieldTable[i].Minlife = UI.CreateNumberInputField(row10)
	...
	InputFieldTable[i].text13 = UI.CreateLabel(row11).SetText('Max Turns alive (Set to 0 to disable life range for Units)').SetColor('#dbddf4')
